...
class Nimegami : MainAPI() {
    ...
    private fun Element.toSearchResult(): AnimeSearchResponse? {
        ...
        val episode =
                this.selectFirst("ul li:contains(Episode), div.eps-archive")
                        ?.ownText()
                        ?.filter { it.isDigit() }
                        ?.toIntOrNull()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        for (i in 1..2) {
            val res =
                    app.get("$mainUrl/page/$i/?s=$query&post_type=post")
                            .document
                            .select("div.archive article")
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val episodes =
                document.select("div.list_eps_stream li").mapNotNull {
                    val episode =
                            Regex("Episode\\s?(\\d+)")
                                    .find(it.text())
                                    ?.groupValues
